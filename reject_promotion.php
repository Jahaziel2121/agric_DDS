...
if (isset($_GET['id'])) {
    ...
    $promo_q = $conn->query("SELECT user_id FROM promotions WHERE id='$id'");
    if ($promo_q && $promo = $promo_q->fetch_assoc()) {
        $farmer_id = $promo['user_id'];
        
        // Reject promotion
        $conn->query("UPDATE promotions SET status='rejected' WHERE id='$id'");
        
        // Notify the farmer
        $msg = "<i class=\"fas fa-times-circle\"></i> Your promotion payment was rejected. The transaction ID could not be verified. Please try again or contact support.";
